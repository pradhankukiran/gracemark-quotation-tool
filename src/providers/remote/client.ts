/**
 * Typed HTTP wrapper for the Remote REST API (gateway.remote.com/v1).
 *
 ...
 * when `REMOTE_API_TOKEN` is missing (e.g. in unit tests that mock the
 * network layer). Base URL and token are sourced from `getRemoteConfig()`
...
export class RemoteClient {
  ...
  private resolve(): { baseUrl: string; token: string } {
    ...
    if (
      this.baseUrlOverride !== undefined &&
  ...
  private async request<T>(
    ...
    try {
      ...
      throw new ProviderError({
        ...
        status: 504,
        cause: err,
        message: `Remote API request to ${path} failed`,
      });
    }
    ...
    if (!response.ok) {
      ...
      throw new ProviderError({
        ...
        cause: detail,
        message: `Remote API ${response.status} ${path}: ${detail}`,
      });
    }
